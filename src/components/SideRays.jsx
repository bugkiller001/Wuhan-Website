...
void main() {
  ...
  gl_FragColor = color;
      ...
      const loop = t => {
        ...
        try { renderer.render({ scene: mesh }); animationIdRef.current = requestAnimationFrame(loop); } catch (e) { return; }
      };
      ...
      cleanupRef.current = () => {
        ...
        if (renderer) {
          try { const loseCtx = renderer.gl.getExtension('WEBGL_lose_context'); if (loseCtx) loseCtx.loseContext(); const c = renderer.gl.canvas; if (c?.parentNode) c.parentNode.removeChild(c); } catch (e) {}
        }
  ...
  useEffect(() => {
    ...
    u.iSpeed.value = speed; u.iRayColor1.value = hexToRgb(rayColor1); u.iRayColor2.value = hexToRgb(rayColor2);
    u.iIntensity.value = intensity; u.iSpread.value = spread;
    const [fx, fy] = originToFlip(origin); u.iFlipX.value = fx; u.iFlipY.value = fy;
    u.iTilt.value = tilt; u.iSaturation.value = saturation; u.iBlend.value = blend;
    u.iFalloff.value = falloff; u.iOpacity.value = opacity;
