...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    setContainerHeight(Math.max(...colHeights) + 12);
    return result;
  }, [columns, items, width]);
    ...
    <div ref={containerRef} className="list" style={{ height: containerHeight || 'auto' }}>
      {grid.map((item, index) => (
        <div
          key={item.id}
          data-key={item.id}
          className={`item-wrapper${itemClassName ? ' ' + itemClassName : ''}`}
          onClick={() => onItemClick ? onItemClick(item, index) : item.url && item.url !== '#' ? window.open(item.url, '_blank', 'noopener') : null}
